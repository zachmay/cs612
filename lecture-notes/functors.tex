...
\begin{notelist}
    \item \code{Maybe} is the Haskell version of the \textit{option type}. It offers us a way to
...
a value of the form \code{Just x} to get at \code{x}? It isn't difficult in principle:
...
\begin{notelist}
    ...
    \begin{notelist}
        \item In \code{example1}, we're making an assumption about how to handle the ``error'' case
...
Let's look at some \code{Functor} instances:
...
\begin{notelist}
    ...
    \begin{notelist}
        ...
        \item Interestingly, while mapping over elements in a set seems reasonable enough, Haskell's \code{Set} type
              can't be directly declared an instance of \code{Functor}. Because \code{Set} is implemented via balanced
    ...
    \item \textbf{\code{((->) e)}}
    \begin{notelist}
        \item This type looks a big strange. Haskell's syntax doesn't allow it, but read this type as \code{(e ->)}.
...
\begin{notelist}
    \item This implementation of \code{fmap} ``doubles'' all the output values: \code{fmap (+1) [1, 2, 3]} returns
          \code{[2, 2, 3, 3, 4, 4]}.
